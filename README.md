...
```bash
# 0단계: Xcode CLI Tools (git 포함) — 직접 설치
...
> **Linux 참고**: `install.sh`에서 Node.js(LTS)와 Claude Code CLI를 자동 설치합니다. macOS는 Xcode CLI Tools를 먼저 직접 설치해야 합니다 (`xcode-select --install`).
...
| ai-session | Claude·Codex account profile 선택·격리 launcher ([계정 프로필 계약](docs/session-account-profiles.md)) | O | O | O |
