...
본다. 이때 메타데이터는 실측과 어긋난 것이므로 그 사실과 실측 위치를 보고한다. 복구하지
않는다. 이 스킬은 `window-token`과 `target-window`를 쓰지 않으며(워크트리 식별자 절),
다른 handle의 창에 얹힌 pane에 그 창의 토큰을 맞추면 두 handle이 같은 창을 가리키게
된다. 매칭된 pane이 다른 handle의 창에 얹혀 있으면 그 창은 이 worktree의 창이 아니다.
기존 위치 안내나 이동 같은 창 단위 조작 전에 독립 창으로 분리할지 확인받고, 아래
분기표의 이동 행을 그대로 적용하지 않는다. 창이 닫혀 있다는 확정은 매칭을 신뢰할 수 있는
상태에서 pane이 0건일 때만 한다. 경로에 공백이 있거나 pane cwd가 바뀐 것으로 의심되면
판정 불가로 두고 저장된 세션·창 후보를 제시해 확인받는다.
