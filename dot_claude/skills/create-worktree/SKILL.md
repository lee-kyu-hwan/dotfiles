...
tmux를 직접 조작하지 않는다. workmux 윈도우는 항상 `main` 세션에 연다.
...
window mode의 모든 `workmux add`와 `workmux open` 호출에
`--parent-session main`을 넘긴다. 실행 중인 에이전트가 `quick` 같은 다른 세션에
있어도 현재 세션을 부모로 사용하지 않는다.
...
확인한다. 실제 윈도우가 `main` 세션에 있는지도 함께 검증한다. 추측한 이름을 안내하면
사용자가 `workmux close`에 없는 이름을 넘기게 된다.
