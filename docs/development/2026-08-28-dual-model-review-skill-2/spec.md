...
- R6.3 결과를 `agreed`(두 리뷰어 합의), `disputed`(반박으로 갈린 지점), `unresolved`(근거 부족으로 미결), `single_source`(리뷰어가 하나뿐이라 합의·불일치를 판정할 수 없음)로 분류한다. `disputed`와 `unresolved`는 버리지 않고 "두 리뷰어가 갈린 지점"으로 게시물에 남긴다. 실행 형태에 따라 산출 가능한 분류가 다르다.
  ...
  | 단일 리뷰어(R3.5 `excluded` 또는 R3.7 승인) | `single_source`, `unresolved` — `agreed`·`disputed`를 산출하지 않는다 |
...
- AC-54 `--base`가 PR의 실제 base와 다르면 `plan`의 `inline_review.skip`이 참이 되고 모든 finding이 `summary_only_findings`로 들어가며, 요약 본문에 base 불일치 사실과 두 ref가 포함된다. 같으면 정상 경로로 동작한다. 검증: 단위 테스트.
...
| R5.4 | AC-22 | R9.2 | AC-28, AC-38 |
| R6.1 | AC-23, AC-24 | R10.1 | AC-25 |
| R6.2 | AC-42 | R10.2 | AC-25 |
...
리뷰어 하나가 실패한 경우 REVIEW 단계에서 추가 승인 게이트가 발생한다(R3.7). 이것과 게시 승인(R7.2)이 이 스킬의 두 승인 게이트다.
...
- `test_publish_findings.py` — AC-1~AC-3, AC-6~AC-16, AC-39, AC-46, AC-47, AC-49, AC-51, AC-52, AC-54. GitHub 접근은 호출 기록을 남기는 fake 클라이언트로 대체하고, 화이트리스트 대조·verdict 고정·쓰기 호출 0건·페이지 순회 완전성을 그 기록으로 판정한다.
- `test_review_state.py` — AC-4, AC-5, AC-17~AC-25, AC-45(프롬프트 구성 함수 출력), AC-50, AC-53. 임시 git 저장소 픽스처를 만들어 파일·라인·diff 범위를 실측한다.
- `test_content_contracts.py` — AC-26, AC-28~AC-33, AC-38, AC-40~AC-45(계약 문구), AC-48. 네 스키마의 루트 형태와 필드 요건(critique의 `evidence` minItems, synthesis의 다섯 축·세 분류 enum), `SKILL.md` frontmatter·`version` 형식·플래그·프리플라이트·두 게이트 문구와 참조 경로, 금지 플래그 부재, 구조화 출력 금지 구성(`uniqueItems`·lookaround) 부재, 표준 라이브러리 import, 토큰 문자열 부재, 디렉터리 구성, 유지보수 문서 절 구성, 종합자 계약의 한계 명시를 확인한다.
