...
**결정적 확인 실험 — 수행했고 원인이 확정됐다 (2026-08-28).**

동일한 `quality-reviewer` 에이전트를 **`name` 없이** 기동했다. 다른 변수는 모두 고정했다
— 같은 `subagent_type`, 같은 모델(opus), 같은 읽기 전용 도구, 리뷰어 계약이 규정한 BLOCKED
payload 경로(아티팩트 입력 누락)를 써서 작고 빠른 산출물만 나오게 했다.

**결과: 최종 JSON이 Agent 반환값으로 그대로 도착했다.** 트랜스크립트 파싱이 필요 없었다.
반환된 payload는 스키마 검증을 통과했다(`{"valid":true,"errors":[]}`). 소요 7.8초,
tool_uses 2회, subagent_tokens 11,022.

meta 비교가 원인을 확정한다:

| 에이전트 | `name` | `taskKind` | 산출물 전달 |
|---|---|---|---|
| 라운드 1 | `spec-review-r1` | `in_process_teammate` | ❌ 트랜스크립트에서 회수 |
| 라운드 1 재시도 | `spec-review-r1-retry` | `in_process_teammate` | ❌ 트랜스크립트에서 회수 |
| 라운드 2 | `spec-review-r2` | `in_process_teammate` | ❌ 트랜스크립트에서 회수 |
| **확인 실험** | **없음** | **`None`** | ✅ **Agent 반환값으로 도착** |

`name`을 넘기면 `taskKind`가 `in_process_teammate`가 되고 최종 메시지가 호출자에게
반환되지 않는다. `name`을 넘기지 않으면 비동기 태스크로 기동되어 완료 통지와 함께 결과가
반환된다. **바뀐 변수는 `name` 하나뿐이다.**

기동 시점의 안내 문구도 다르다 — 이름 있는 경우는 teammate 등록과 mailbox 안내가 나오고,
이름 없는 경우는 비동기 태스크 기동과 완료 자동 통지 안내가 나온다.

**수정 방향이 확정됐다.** 리뷰어 도구를 늘리는 것이 아니라, `SKILL.md`의 리뷰 호출
계약에서 리뷰 라운드 기동 시 `name`을 넘기지 않도록 규정하고 그 계약을 content 테스트로
고정한다. 읽기 전용 리뷰어(`tools: Read, Grep, Glob`)는 의도된 설계이며 그대로 유지된다.
