...
- Created: 2026-08-28
- Updated: 2026-09-04
- 판본 이력: 2026-08-28에 선행 실행(`20260828T021938Z-...`)의 Spec 라운드 2에서 93점 PASS를 받은 본을 이 실행의 초안으로 재사용했다. 그 통과는 이 실행의 게이트 판정과 무관하며, 현재 본은 이 실행의 라운드 1 지적 12건을 반영한 개정본이다. Task ID 타임스탬프(`20260903T160637Z`)는 이 실행의 생성 시각, 디렉터리 날짜(`2026-09-04`)는 산출물 배치 날짜, `Updated`는 현재 개정 날짜다.
...
- R2.1 실행 시작 시 대상 저장소(`owner/name`), PR 번호, `base_sha`, `head_sha`, 변경 파일 목록 다섯 값을 상태 파일에 고정한다. 이후 모든 단계는 상태의 이 값만 참조하고 GitHub에 다시 묻지 않는다(예외: R7.11의 게시 직전 head SHA 재확인).
...
- R2.5 INTAKE의 순서는 이렇다: PR을 조회해 `repo`·`pr_number`·`base_sha`·`head_sha`를 메모리에 얻고 → 그것으로 `run_id`를 만들고 → 상태 디렉터리를 열거나 만들고 → R2.1의 다섯 값을 그 안의 `state.json`에 고정한다. `run_id`는 `<repo_owner>-<repo_name>-pr<pr_number>-<head_sha 앞 12자>` 형식으로 **결정적으로** 생성한다. 같은 PR의 같은 head SHA에 대한 재실행은 같은 `run_id`를 얻어 직전 실행의 상태 디렉터리를 그대로 연다. head SHA가 바뀌면 다른 `run_id`가 되고 이전 상태를 재사용하지 않는다 — 리뷰 대상이 달라졌기 때문이다. 이 규칙이 R7.13(a)의 "완료 기록된 단계는 다시 실행하지 않는다"를 성립시킨다. 상태 디렉터리가 이미 있으면 그 안의 `state.json`을 읽어 이어가고, 없으면 새로 만든다. 별도의 재개 플래그를 두지 않는다.
...
- R3.9 **양쪽 리뷰어가 모두 실패하면**(R3.7의 셋 중 어느 조합이든) 게시하지 않고 중단한다. 단일 리뷰 승인을 묻지 않는다 — 승인해도 종합할 산출물이 없다. Claude 측 에이전트가 **일부만** 실패한 경우는 리뷰어 실패가 아니며, 나머지 에이전트로 진행하되 실패한 에이전트 목록과 그것이 담당했을 카테고리를 상태·리포트·게시 요약에 남긴다(R7.6a의 커버리지 결손 처리 대상이 된다).
...
- R6.3 결과를 `agreed`(두 리뷰어 합의), `disputed`(반박으로 갈린 지점), `unresolved`(근거 부족으로 미결), `single_source`(리뷰어가 하나뿐이라 합의·불일치를 판정할 수 없음)로 분류한다. `disputed`와 `unresolved`는 버리지 않고 "두 리뷰어가 갈린 지점"으로 게시물에 남긴다. 실행 형태에 따라 산출 가능한 분류가 다르다.

  | 실행 형태 | 산출 가능한 분류 |
  |---|---|
  | 두 리뷰어 + 교차비평(기본) | `agreed`, `disputed`, `unresolved` |
  | 두 리뷰어 + `--rounds 0` | `agreed`, `unresolved` — 반박 단계가 없으므로 `disputed`를 산출하지 않는다 |
  | 단일 리뷰어(R3.7의 세 실패 유형 중 하나가 발생하고 단일 리뷰 승인이 기록된 실행. `excluded`도 승인을 거친다) | `single_source`, `unresolved` — `agreed`·`disputed`를 산출하지 않는다 |

  단일 리뷰어 실행에서 `single_source`가 붙은 finding은 "교차 검증되지 않음"을 뜻하며, 근거 부족을 뜻하는 `unresolved`와 구별된다. 이 구별을 게시 요약에 명시한다.
...
- R7.6 lifecycle 분류: 이전 게시에 없으면 `new`, 이전에도 있고 지금도 있으면 `persisting`(새 댓글 생성 금지), 이전에 있었으나 지금 없으면 **R7.6a의 커버리지 판정을 거쳐** `resolved` 또는 `not_re_reviewed`.
- R7.6a **커버리지 결손 하에서는 `resolved`로 분류하지 않는다.** finding이 이번 실행의 결과에서 사라지는 원인은 실제 해소만이 아니다. 이 스킬 자신이 규정한 결손 경로가 셋 있고, 그 경로에 걸린 finding은 "고쳐졌는지 다시 보지 않은 것"이지 "고쳐진 것"이 아니다.

  | 결손 경로 | 판정 |
  |---|---|
  | R10.2(b)로 축소된 범위 밖의 경로에 있던 finding | `not_re_reviewed` |
  | `excluded` 리뷰어(R3.5)가 단독으로 냈던 finding | `not_re_reviewed` |
  | 실패한 Claude 에이전트(R3.9)가 담당하는 카테고리의 finding | `not_re_reviewed` |

  판정 순서는 결정적이다: 이전 게시에 있고 지금 없는 finding에 대해 위 세 조건을 차례로 확인하고, 하나라도 걸리면 `not_re_reviewed`, 전부 해당하지 않을 때만 `resolved`다. 판정은 스크립트가 수행하며 근거(어느 경로에 걸렸는지)를 상태에 기록한다.
- R7.7 lifecycle 별 게시 동작은 다음과 같다.
  ...
  | `resolved` | "해소됨" 목록에 기재 | `resolveReviewThread`로 해결 |
...
- R8.2 `scripts/publish_findings.py`는 `plan`/`apply` 서브커맨드로 게시 계획 산출과 게시를 담당한다. GitHub 접근은 주입 가능한 클라이언트 인터페이스를 거쳐 테스트에서 fake로 대체할 수 있어야 하며, 인터페이스는 정확히 다음 아홉 메서드만 노출한다. 각 메서드는 호출을 R7.14의 3튜플로 기록한다.
  ...
  **세 목록 조회 메서드(`list_issue_comments`, `list_review_comments`, `list_review_threads`)는 전체 페이지를 순회해 완전한 목록을 반환한다.** GitHub의 목록 응답은 기본적으로 부분 페이지이고, 게시 멱등성 전체가 "기존 게시물에서 `finding_id` 마커를 빠짐없이 찾는 것"에 의존하므로 부분 조회는 곧 재게시와 `resolved` 오분류를 뜻한다. REST 두 메서드는 `per_page`를 최대로 두고 `Link` 헤더의 `rel="next"`가 없어질 때까지(또는 `gh api --paginate`로) 순회하고, GraphQL 메서드는 `pageInfo.hasNextPage`가 거짓이 될 때까지 커서를 따라간다. 순회가 중간에 실패하면 부분 목록을 반환하지 않고 오류를 올린다 — 불완전한 목록으로 계획을 세우는 것이 조용한 재게시보다 나쁘다.
...
- AC-4 R4.2의 네 조건 각각에 대해 `location_valid=false`가 되고 inline 게시 계획에서 제외된다: 존재하지 않는 파일, 라인 수를 초과하는 `line_start`, **라인 수를 초과하는 `line_end`**, **`line_start > line_end`인 역전**. 네 조건을 모두 만족하지 않는 finding만 `location_valid=true`가 된다. 검증: 네 조건 각각의 픽스처를 갖는 임시 git 저장소 단위 테스트.
...
- AC-10 **커버리지 결손이 없는 실행**(범위 축소 없음, `excluded` 리뷰어 없음, 실패 에이전트 없음)에서 이전 게시에 있었고 현재 리뷰에 없는 `finding_id`는 `resolved`로 분류되며, 계획에 요약의 "해소됨" 기재와 `resolveReviewThread` 호출이 포함되고 스레드 답글 호출은 포함되지 않는다. 검증: 단위 테스트.
...
- AC-13 fake 클라이언트가 수신한 모든 리뷰 생성 호출의 `event` 값이 `COMMENT`다. 추가로 계약 테스트가 `publish_findings.py` 소스를 읽어 문자열 `REQUEST_CHANGES`와 `APPROVE`의 출현 횟수가 각각 0임을 단정한다. 종료 코드가 아니라 카운트를 단정하는 이유는 `grep -c`가 매치 0건일 때 `0`을 출력하면서 종료 코드 1을 반환해 두 기준이 충돌하기 때문이다. 검증: 단위 테스트 + 계약 테스트.
...
- AC-21 새 근거 수가 0인 라운드 뒤에는 다음 라운드가 실행되지 않고 종료 사유 `no_new_evidence`가 기록된다. 라운드 수는 2를 초과할 수 없다. **첫 교차비평 라운드의 기준선 집합이 1차 리뷰 findings의 `evidence` 튜플을 포함해, 1차 리뷰가 이미 댄 근거만 되풀이한 반박에 대해 첫 라운드에서 `no_new_evidence`가 성립한다.** 검증: 단위 테스트.
- AC-22 R5.4의 두 조건이 모두 참인 입력에서 `abstraction_drift`가 반환되고, 하나라도 거짓이면 반환되지 않는다. **직전 교차비평 라운드가 없는 첫 라운드 입력에서는 첫 조건이 참이어도 반환되지 않는다.** 검증: 단위 테스트.
...
- AC-25 R10.1 임계값을 넘는 입력에서 파이프라인이 사용자 결정 없이 다음 단계로 전이하지 않고, 범위를 축소한 경우 축소된 경로 집합과 제외 파일 수가 상태에 기록되며 **`plan`이 생성한 요약 코멘트 본문에도 축소 사실과 제외 파일 수가 포함된다**. 검증: 단위 테스트.
...
- AC-33 `dot_claude/skills/dual-review/` 아래에 R1.2가 규정한 다섯 구성이 모두 존재하고(`SKILL.md` 파일, `references/`·`schemas/`·`scripts/`·`tests/` 디렉터리), `templates/`와 `evals/` 디렉터리는 없다. 검증: 계약 테스트.
...
- AC-43 `schemas/synthesis.schema.json`이 각 finding의 분류를 `agreed`/`disputed`/`unresolved`/`single_source` 네 값의 `enum`으로 강제하고, 그 밖의 값을 가진 픽스처는 검증에 실패한다. 검증: 유효·무효 픽스처 단위 테스트.
...
- AC-46 생성된 모든 코멘트 원소에서 **`line`이 finding의 `line_end`와 같고 `side`가 `"RIGHT"`다.** 그리고 `line_start == line_end`인 finding의 원소에는 `start_line`·`start_side`가 없고, `line_start < line_end`이면서 두 라인이 같은 hunk 안인 finding의 원소에는 `start_line == line_start`와 `start_side`가 있으며, 두 라인이 서로 다른 hunk에 있으면 `start_line`이 없고 축소 사실이 상태에 기록된다. 어떤 원소에도 `position` 키가 없다. 검증: 단위 테스트.
- AC-47 기존 리뷰 코멘트 조회 결과로부터 `id`·`node_id`·`pull_request_review_id`·`path`·`line`·`original_line`이 상태에 기록된다. 검증: fake 클라이언트 단위 테스트.
- AC-55 `--pr`가 없을 때 현재 브랜치의 PR을 조회해 대상으로 삼고, 조회 결과가 비면 상태를 만들지 않고 종료 코드 != 0으로 중단한다. 검증: fake 클라이언트 단위 테스트(PR 있음/없음 두 경우).
- AC-56 `abstraction_drift` 신호가 참인 상태에서 사용자 결정이 상태에 기록되기 전에는 다음 교차비평 라운드도 SYNTHESIS 전이도 일어나지 않는다. 검증: 단위 테스트.
- AC-57 R10.1 임계값 초과 시 사용자가 중단을 선택하면 상태가 중단으로 기록되고 이후 단계가 실행되지 않으며 GitHub 쓰기 호출이 0건이다. 검증: 단위 테스트.
- AC-58 `viewerCanResolve`가 거짓인 스레드에 대해 `resolveReviewThread` 호출이 발생하지 않고, 건너뛴 스레드 ID와 사유가 상태에 기록되며 요약 본문에 포함된다. 참인 스레드는 정상 호출된다. 검증: fake 클라이언트 단위 테스트.
- AC-59 양쪽 리뷰어가 모두 실패한 실행에서는 단일 리뷰 승인을 묻지 않고 중단하며 GitHub 쓰기 호출이 0건이다. 검증: 단위 테스트.
- AC-60 `references/reviewer-contract.md`에 기록된 상수가 스크립트가 실제로 쓰는 값과 일치한다: `CODE_EXT` 집합, 확장자별 주석 토큰 표, `comment-analyzer` 매치 임계 3, 규모 임계값 파일 100개·diff 20,000줄. 검증: 계약 테스트가 문서에서 값을 파싱해 스크립트 상수와 대조한다.
- AC-61 같은 `repo`·`pr_number`·`head_sha` 입력에 대해 `run_id`가 재현되고, `head_sha`만 다르면 달라진다. 기존 상태 디렉터리가 있으면 `state.json`을 읽어 이어가고 없으면 새로 만든다. 검증: 단위 테스트.
- AC-62 R7.6a의 세 결손 경로 각각에 대해, 이전 게시에 있고 현재 결과에 없는 finding이 `resolved`가 아니라 `not_re_reviewed`로 분류되고, 그 finding의 스레드에 `resolveReviewThread` 호출이 발생하지 않으며, 요약 본문의 "재검토되지 않음" 목록에 사유와 함께 나타난다. 세 조건 모두 해당하지 않을 때만 `resolved`가 된다. 검증: 세 결손 경로 각각의 fake 클라이언트 단위 테스트.
- AC-63 R3.7이 열거한 세 실패 유형 각각에서 단일 리뷰 승인이 상태에 기록되기 전에는 CRITIQUE·SYNTHESIS로 전이하지 않는다. 특히 R3.5의 `excluded` 경로도 승인 게이트를 발동한다. 검증: 세 유형 각각의 단위 테스트.
- AC-48 Codex 호출 계약 문서와 프롬프트·명령 구성 함수의 출력에 `--sandbox read-only`가 존재하고, `--sandbox`의 다른 값(`workspace-write`, `danger-full-access` 등)이 스킬 디렉터리 어디에도 등장하지 않으며, R3.2가 열거한 `--ephemeral`·`--output-schema`·`--output-last-message`·`--json`·`--model`·`-c model_reasoning_effort`가 모두 존재한다. 검증: 계약 테스트.
- AC-49 목록 조회 메서드 셋이 전체 페이지를 순회한다. 2페이지에 걸친 응답을 반환하는 fake 클라이언트에서, **2페이지째에만 `finding_id` 마커가 있는 finding이 `persisting`으로 분류되어 재게시되지 않고**, 같은 조건에서 `resolved` 판정도 오분류되지 않는다. 순회 중 오류가 나면 부분 목록을 반환하지 않고 예외가 전파된다. 검증: 다중 페이지 fake 클라이언트 단위 테스트.
- AC-50 실행 형태별로 산출 가능한 분류가 R6.3의 표와 일치한다: 두 리뷰어 + 교차비평은 `single_source`를 산출하지 않고, `--rounds 0`은 `disputed`를 산출하지 않으며, 단일 리뷰어 실행은 `agreed`·`disputed`를 산출하지 않는다. 검증: 세 실행 형태 각각의 단위 테스트.
- AC-51 INTAKE 후 상태에 `repo`·`pr_number`·`base_sha`·`head_sha`·`changed_files` 다섯 값이 모두 기록되고, 이후 단계의 함수들이 GitHub 재조회 없이 상태의 값만 사용한다(예외: R7.11의 게시 직전 head SHA 재확인 1회). 검증: fake 클라이언트 호출 기록으로 재조회 부재를 확인하는 단위 테스트.
- AC-52 `schemas/publish-plan.schema.json`을 만족하지 않는 계획 파일로 `apply`를 실행하면 종료 코드가 0이 아니고 GitHub 쓰기 호출이 0건이다. 검증: fake 클라이언트 단위 테스트.
- AC-53 `--rounds 0`으로 실행하면 교차비평 라운드가 0회이고 critique 산출물이 생성되지 않으며, 상태의 `critique_rounds`가 빈 목록이다. 검증: 단위 테스트.
- AC-54 `--base`가 PR의 실제 base와 다르면 (a) 상태에 base 불일치 사실과 두 ref(`requested_ref`, `actual_base_sha`)가 기록되고, (b) `plan`의 `base_mismatch`가 non-null이며 그때 `inline_review.skip`이 반드시 참이고, (c) 모든 finding이 `summary_only_findings`로 들어가며, (d) 요약 본문에 base 불일치 사실과 두 ref가 포함된다. 같으면 `base_mismatch`가 null이고 정상 경로로 동작한다. 검증: 단위 테스트.
...
| R2.3 | AC-32 | R7.6a | AC-62 |
| R2.4 | AC-36 | R7.7 | AC-10, AC-62 |
| R2.5 | AC-61 | R7.8 | AC-12 |
...
| R3.9 | AC-59 | R7.17 | AC-47 |
| R4.1 | AC-4 | R7.18 | AC-58 |
| R4.2 | AC-4 | R8.1 | AC-4, AC-5, AC-17~AC-25 |
| R4.3 | AC-5 | R8.2 | AC-14, AC-15, AC-49 |
...
| R6.1 | AC-23, AC-24 | R10.1 | AC-25, AC-60 |
| R6.2 | AC-42 | R10.2 | AC-25, AC-57 |
| R6.3 | AC-43, AC-50 | R10.3 | AC-25 |

AC-27(codex 구조화 출력 수락)과 AC-34(전체 결정적 테스트 통과)는 개별 요구사항이 아니라 R3.8·R8.5의 실행 가능성과 스위트 전체를 검증하는 메타 기준이므로 표에 별도 행을 두지 않는다. 매핑은 요구사항 문언을 실제로 판정하는 기준만 싣는다 — 관련은 있으나 그 문언을 단정하지 못하는 AC는 등재하지 않는다.
...
| `schemas/synthesis.schema.json` | 종합 산출 (루트 object). 다섯 축 required, 분류 4값 enum | JSON Schema 2020-12 |
...
| `lifecycle` | object | `{new: [...], persisting: [...], resolved: [...], not_re_reviewed: [{finding_id, reason}]}`. `reason`은 R7.6a의 세 결손 경로 중 하나 |
...
| 커버리지 결손으로 finding이 사라짐 | `resolved`가 아니라 `not_re_reviewed`로 분류하고 스레드를 건드리지 않으며 요약에 사유와 함께 남긴다(R7.6a). |
...
- `test_publish_findings.py` — AC-1~AC-3, AC-6~AC-16, AC-39, AC-46, AC-47, AC-49, AC-51, AC-52, AC-54, AC-55, AC-57, AC-58, AC-62. GitHub 접근은 호출 기록을 남기는 fake 클라이언트로 대체하고, 화이트리스트 대조·verdict 고정·쓰기 호출 0건·페이지 순회 완전성을 그 기록으로 판정한다.
- `test_review_state.py` — AC-4, AC-5, AC-17~AC-25, AC-45(프롬프트 구성 함수 출력), AC-50, AC-53, AC-56, AC-59, AC-61, AC-63. 임시 git 저장소 픽스처를 만들어 파일·라인·diff 범위를 실측한다.
- `test_content_contracts.py` — AC-26, AC-28~AC-33, AC-38, AC-40~AC-45(계약 문구), AC-48, AC-60. 네 스키마의 루트 형태와 필드 요건(critique의 `evidence` minItems, synthesis의 다섯 축·세 분류 enum), `SKILL.md` frontmatter·`version` 형식·플래그·프리플라이트·두 게이트 문구와 참조 경로, 금지 플래그 부재, 구조화 출력 금지 구성(`uniqueItems`·lookaround) 부재, 표준 라이브러리 import, 토큰 문자열 부재, 디렉터리 구성, 유지보수 문서 절 구성, 종합자 계약의 한계 명시를 확인한다.
...
이 실행의 라운드 1이 낸 12건을 반영하면서, 개별 해소가 서로 만드는 틈을 점검했다. 선행 실행에서 라운드 2의 신규 지적 셋이 전부 라운드 1 해소 과정의 회귀였기 때문이다.

| 겹치는 지점 | 관련 finding | 점검 결과 |
|---|---|---|
| R3.5 · R3.7 · R3.9 · R6.3 · Architecture | SPEC-03(실패 경계), SPEC-04(두 리뷰어 실패에 번호), SPEC-09(`excluded`가 결손 경로) | 네 곳을 함께 다시 썼다. `excluded`가 R3.7 게이트를 발동하도록 확정하자 R6.3 표의 "R3.5 `excluded` 또는 R3.7 승인" 표현이 낡아 두 경로를 대등하게 보이게 했다 — **교차 점검에서 발견해 함께 고쳤다.** R3.9의 "에이전트 일부 실패"가 R7.6a의 세 번째 결손 경로와 같은 사건이라는 것도 양쪽에 상호 참조로 명시했다 |
| R7.6 · R7.6a · R7.7 · AC-10 · AC-62 | SPEC-09 | AC-10을 "커버리지 결손 없는 실행"으로 좁히고 AC-62가 결손 경로를 맡아 둘이 상보적이다. 추적표에서 R7.6·R7.7이 둘 다 두 AC를 참조한다 |
| R1.4 · R2.5 | SPEC-04(중단 AC), SPEC-12(`run_id`) | R2.5가 "별도 재개 플래그를 두지 않는다"로 확정돼 R1.4의 네 플래그 집합이 그대로 유지된다. 충돌 없음 |
| R2.1 · R2.5 | SPEC-12 | `run_id`를 만들려면 `head_sha`가 필요하고 `head_sha`를 저장하려면 상태 파일이 필요해 순환처럼 보였다. **교차 점검에서 발견해** INTAKE 순서(조회 → `run_id` → 디렉터리 → 고정)를 R2.5에 명시했다 |
| R7.16 · D18 | SPEC-10 | R7.16 본문의 근거를 "보수적 선택"으로 정정하면서 D18이 여전히 축소 규칙을 실측의 결론으로 읽히게 뒀다. **교차 점검에서 발견해** D18에서 필드 구성과 축소 규칙을 분리했다 |
| publish-plan 스키마 | SPEC-09(`not_re_reviewed`), SPEC-04(`skipped_threads`) | 두 필드를 함께 추가했다. `viewerCanResolve` 조회는 기존 `list_review_threads`(QUERY)로 충분해 엔드포인트 화이트리스트는 바뀌지 않는다 |
| 추적표 | SPEC-01·02·04·05·09·11·12 | 표를 부분 수정하지 않고 60행 전체를 재생성했다. 검증: 요구사항 60건 전수 등재(누락 0·유령 0), 표가 참조하는 AC 63건 전부 실재, Test strategy 미배정 AC 0건 |

교차 점검에서 새로 발견해 함께 고친 것이 셋이다(R6.3 표현, R2.1/R2.5 순서, D18 분리). 어느 것도 개별 finding에는 없었고 해소들의 교차점에서만 드러났다.
...
- D18. inline 코멘트의 **필드 구성**은 실측으로 확정했다. 다만 R7.16의 hunk 경계 축소 규칙은 이 실측의 결론이 아니라 별도의 보수적 선택이며 그 근거는 R7.16 본문에 있다. `zambaguni/zambaguni-front`의 PR #1255·#1211·#1313에서 리뷰 코멘트 79건의 응답 키 집합을 조회해(본문·코드 내용은 취득하지 않고 키 이름만) 세 PR 모두 동일한 구성임을 확인했다: `path`, `line`, `side`, `start_line`, `start_side`, `body`, `commit_id`, `id`, `node_id`, `pull_request_review_id`, `original_line`, `original_start_line`, `subject_type`, `diff_hunk`, 그리고 deprecated인 `position`·`original_position`. 이에 따라 R7.16·R7.17을 확정했고, 여러 줄 코멘트를 지원하되 hunk 경계를 넘으면 단일 라인으로 축소한다.

- D19. 종합 분류에 네 번째 값 `single_source`를 둔다. 리뷰어가 하나뿐인 실행에서 `agreed`(합의)도 `disputed`(불일치)도 원리상 성립하지 않는데 세 값 enum을 강제하면 모든 finding이 `unresolved`로 일괄 강등되어, "근거가 부족하다"와 "교차 검증할 상대가 없었다"가 구별되지 않는다. 후자는 리뷰 품질에 대한 정보이므로 별도 값으로 보존한다(R6.3, AC-50).
- D20. 목록 조회는 전체 페이지를 순회한다(R8.2). 게시 멱등성 전체가 마커 완전 조회에 의존하는데 GitHub 목록 응답은 기본이 부분 페이지다. 순회 실패 시 부분 목록을 반환하지 않고 오류를 올리는 쪽을 택했다 — 불완전한 목록으로 세운 계획은 조용한 재게시와 `resolved` 오분류를 낳고, 그것이 실패를 드러내는 것보다 나쁘다. 이 결함은 단일 페이지 fake로는 구조적으로 검출되지 않으므로 AC-49가 다중 페이지 fake를 명시적으로 요구한다.

- D21. `resolved`와 `not_re_reviewed`를 분리한다(R7.6a). 대안은 `resolved` 하나로 두고 요약에 주의 문구만 남기는 것이었으나, 그러면 `resolveReviewThread`가 여전히 호출되어 미해소 지적의 스레드가 닫힌다. 스레드 해결은 되돌리려면 사람이 GitHub에서 직접 열어야 하는 외부 쓰기이므로, 분류 자체를 나눠 호출을 막는 쪽을 택했다. 커버리지 결손 세 경로(R10.2b 범위 축소, R3.5 `excluded`, R3.9의 에이전트 일부 실패)는 전부 이 스킬이 스스로 규정한 것이라 판정이 결정적이다.
...
- D23. `run_id`를 `<owner>-<repo>-pr<n>-<head 12자>`로 결정적으로 만들고 별도 재개 플래그를 두지 않는다(R2.5). 대안은 타임스탬프 기반 ID와 `--resume` 플래그였으나, 그러면 재실행이 직전 상태를 찾으려고 디렉터리를 뒤져야 하고 어느 것을 이어갈지가 모호해진다. head SHA를 ID에 넣으면 "같은 대상이면 같은 상태, 대상이 바뀌면 새 상태"가 규칙 없이 성립한다.
...
통제와 그 검증: 엔드포인트 화이트리스트(AC-14), 승인 게이트(AC-15, AC-20, AC-28), 위치 실측(AC-4, AC-5), verdict 고정(AC-13), 금지 플래그 부재(AC-29), 샌드박스 read-only와 나머지 호출 플래그 존재(AC-48), 토큰 미취급(AC-31).
...
1. **fake 클라이언트 통합 검증(자동).** 게시 lifecycle 전체(new → persisting → resolved → not_re_reviewed), 각 단계 실패 후 재실행 멱등, head SHA 불일치 중단, 화이트리스트 준수, verdict 고정, 커버리지 결손 하 resolve 억제, `viewerCanResolve` 스킵을 fake GitHub 클라이언트로 실행한다. AC-7~AC-16과 AC-58·AC-62가 이 경로다. 중단 조건: 하나라도 실패하면 게시 계약을 고친 뒤 다시 돌린다.
