...
`⌃⌥` + 화살표를 반복해서 누르면 같은 방향의 분할 비율이 순환된다 (Magnet에는 없는 확장 기능).
...
판단 기준은 **현재 윈도우 위치**다 — 정확히 1/2 위치면 다음에 1/3로, 1/3이면 1/4로. 사이클 외 위치(예: 수동으로 옮긴 윈도우)에서 누르면 1/2부터 시작한다. 타이머 기반 상태 추적이 아니라 위치 기반이므로 "키를 빨리 눌러야 한다"는 부담이 없다.
