...
| `prefix` → `=` → `숫자` | 선택한 패인을 상단 메인으로 올리고 레이아웃 정렬 | ★ |

> 기본 분할 키 `%`/`"`도 여전히 동작하지만, `|`/`-`가 더 직관적입니다.
>
> `prefix → =`를 누르면 각 패인에 번호가 표시되고, 이어서 **패인 번호를 누르면** 그 패인이 상단 메인(높이 55%)으로 이동하고 나머지는 하단에 가로로 배치됩니다(`main-horizontal`). 패인이 3개일 때 하단 2개가 좌우 50:50으로 보입니다. 이미 메인인 `0`번을 누르면 자리 이동 없이 레이아웃 정렬만 적용됩니다.
