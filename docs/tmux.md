...
> | 6 | `3 + 3` | … | … |
>
> 패인 순서(왼쪽→오른쪽, 위→아래)는 정렬 전후로 유지됩니다.
>
> 2개는 tmux 내장 `even-horizontal`을 그대로 사용하므로, 창 너비에 따라 두 패인의 너비가 최대 1칸까지 차이날 수 있습니다. 나머지는 tmux 기본 레이아웃에는 없는 배치라, `~/.local/bin/tmux-stack-layout` 스크립트가 패인 개수에 맞춰 레이아웃 문자열을 동적으로 생성한다(`dot_local/bin/`로 관리).
