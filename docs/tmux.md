...
>
> 전부 `~/.local/bin/tmux-stack-layout` 스크립트가 패인 개수에 맞춰 레이아웃 문자열을 동적으로 생성합니다(`dot_local/bin/`로 관리). 2개의 결과는 tmux 내장 `even-horizontal`과 같지만 내장을 쓰지 않습니다 — 문자열에는 패인 수와 ID가 박혀 있어, 생성과 적용 사이에 패인이 늘거나 줄면 `have N panes but need M`으로 거부됩니다. 내장은 적용 시점의 창 상태를 그대로 따라가 그런 변화를 조용히 삼킵니다.
>
