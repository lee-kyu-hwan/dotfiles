...
- 원본에 agent가 추가되거나 삭제되어 집합이 달라지면 조용히 무시하지 않고 검증을
  실패시킨다.
- **이 검증의 적용 범위는 구현 시점의 parity 검사다.** 런타임 요구와 혼동하면 안 된다 —
  오케스트레이터 `SKILL.md`는 "upstream이 agent를 추가하는 것은 실패가 아니다"라고
  명시한다(이 실행이 dispatch하지 않는 파일이므로). 따라서 upstream **추가**로 위
  정확일치 검증이 실패하면 그것은 런타임 장애 신호가 아니라 **래퍼 추가를 검토하라는
  신호**다. **삭제**는 실제 파손이므로 그대로 실패로 다룬다.
