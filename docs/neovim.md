...
| `<Space>fc` | 커서 아래 단어 검색 (grep_string) | ★ |
| `<Space>fb` | 버퍼 목록 | ★ |
| `<Space>fh` | 도움말 검색 | ★ |
| `<Space>fr` | 최근 파일 | ★ |
...
> 설정 파일: `dot_config/nvim/lua/plugins/todo-comments.lua`

코드의 `TODO`, `FIXME`, `HACK`, `BUG`, `NOTE` 등을 컬러로 하이라이트하고 검색/목록으로 관리.

| 키 | 기능 | 구분 |
|----|------|------|
| `]t` | 다음 TODO로 이동 | ★ |
| `[t` | 이전 TODO로 이동 | ★ |
| `<Space>ft` | TODO 검색 (Telescope) | ★ |
| `<Space>xt` | TODO 목록 (Trouble) | ★ |
...
> 설정 파일: `dot_config/nvim/lua/plugins/tmux-navigator.lua`
> tmux 패인과 Neovim 창을 구분 없이 `Ctrl+h/j/k/l`로 이동합니다.

| 키 | 기능 | 구분 |
|----|------|------|
| `<C-h>` | 왼쪽으로 (tmux/vim 공유) | ★ |
| `<C-j>` | 아래로 (tmux/vim 공유) | ★ |
| `<C-k>` | 위로 (tmux/vim 공유) | ★ |
| `<C-l>` | 오른쪽으로 (tmux/vim 공유) | ★ |
...
> 설정 파일: `dot_config/nvim/lua/plugins/lualine.lua`

하단 상태바에 현재 모드, 브랜치, diff, 진단, 파일 경로, 인코딩, 파일 타입 등을 표시. catppuccin 테마 연동.

별도 키맵 없이 자동 표시.

### 들여쓰기 가이드 (indent-blankline.nvim)

> 설정 파일: `dot_config/nvim/lua/plugins/indent-blankline.lua`

들여쓰기 레벨에 세로 가이드라인(`│`)을 표시. 중첩이 깊은 코드에서 스코프 파악에 유용.
