...
For each input record, preserve every normalized field and its JSON type. Exact preservation includes nested JSON value types: numbers and booleans are distinct. Present records in any order, but retain all identities, IDs, normalized values, source ordering/metadata, and normalized histories. Match resolved records by PR node identity and unresolved current records by their observed PR URL. For prior analysis, prefer exact identity matches. A prior unresolved record may transition to exactly one still-unmatched current resolved node with the same observed URL and complete history; zero matches are removal, multiple matches are ambiguity, and URL fallback never applies to a previously resolved identity. Add exactly `analysis` and `analysis_history` as analysis fields.

`analysis` is the current `AnalysisProjection`. `analysis_history` is the exact input or prior-output history prefix plus exactly one `AnalysisSnapshot` for this run. Preserve every old item exactly, including JSON value types, even if it predates the strict shape. The newest snapshot revision equals the envelope revision; its `evidence_manifest` type-aware deep-equals the current projection manifest; its `conclusion` type-aware deep-equals the current projection.
