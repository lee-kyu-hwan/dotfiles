...
1. Run `scripts/validate_corpus.py` on the input (and `--existing` when supplied). Support schema `1.0.0` only. On validation failure or another version, stop and request an explicit migration; do not repair or reinterpret data.
...
Capabilities are local historical analysis and requested local output writing only; never mutate the supplied input in place. Do not retry validation, migration, or missing-evidence failures automatically: stop, report the condition, and await corrected input or an explicit migration. Stop on unsupported schema, invalid corpus, unresolved identity needing an ID, or a preservation conflict.
